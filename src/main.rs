...
####################################################################
    ...
    loop {
        ...
        if timer >= 1.0 / target_fps
        {
            ...
            {
                player_pos.y = (ground_pos) as f32;
                player_velocity.y *= -1.0;
            }

            if (player_pos.x as u16 > wall_right_pos - 1)
            {
                player_pos.x = (wall_right_pos) as f32;
                player_velocity.x *= -1.0;
            }

            if ((player_pos.x as u16) < (wall_left_pos + 1))
            {
                player_pos.x = (wall_left_pos + 1) as f32;
                player_velocity.x *= -1.0;
            }
